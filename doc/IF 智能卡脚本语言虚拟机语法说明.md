...
- VS Code 编辑支持：`.if` 和 `.card` 文件均提供内置对象补全、签名提示和“格式化文档”。保存文件会自动格式化；格式化使用 4 空格缩进，统一运算符、逗号与括号周围的空格，并最多保留一个连续空行。多行列表和哈希表会逐层缩进，连续的行尾 `//` 注释会对齐，`case/default` 前的 `//` 说明与标签对齐。含 `->` 的 APDU 和 `RST` 表达式会合并为一行，不受长度限制。
- 脚本转换：电信、金融脚本转换期间会显示加载进度；只有转换进程结束且未修改的已打开脚本文档完成刷新后，才显示转换成功。
