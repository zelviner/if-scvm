...
赋值和复合赋值表达式返回写入后的新值。`++` 和 `--` 支持整数及浮点数，并会把新值写回变量、索引或属性；作为值表达式时返回修改前的值。
...
使用 fmt 格式语法生成字符串。第一个参数必须是格式字符串，后续参数可以是任意 Card Script 值。整数、浮点数、布尔值和字符串保留其原生格式化能力；`null`、列表、哈希表等其他值使用其字符串表示。

```card-script
format("status={} target=0x{:02X}", 1, 15) // "status=1 target=0x0F"
format("{1}-{0}-{1}", "A", "B") // "B-A-B"
format("{{{:04X}}}", 255) // "{00FF}"
format("|{:>5}|{:<5}|{:^5}|", "A", "B", "C") // "|    A|B    |  C  |"
format("{:.2f} {:08b}", 3.14159, 5) // "3.14 00000101"
```
...
| `{0}`、`{1}` | 按位置插入，可重复使用 |
| `{{`、`}}` | 输出字面量 `{`、`}` |
| `{:02X}` | 大写十六进制，宽度 2，不足补零 |
| `{:08b}` | 二进制，宽度 8，不足补零 |
| `{:.2f}` | 浮点数保留两位小数 |
| `{:>10}`、`{:<10}`、`{:^10}` | 右对齐、左对齐、居中 |
| `{0:{1}.{2}f}` | 使用后续参数动态指定宽度和精度 |

自动位置 `{}` 和手动位置 `{0}` 不能在同一个格式字符串中混用。参数不足、格式字符串不完整或值类型与格式说明不匹配时会产生运行时错误。当前只支持位置参数，不支持 fmt 命名参数。
